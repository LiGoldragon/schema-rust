...
#[test]
fn emits_rust_projection_for_renamed_generic_definitions_available_in_current_api() {
    let schema = FixtureSchema::new("generic-aliases.schema").lower("aliases:lib");
    let generated = RustEmitter::default().emit_code_from_true_schema(&schema);
    let code = generated.as_str();

    assert!(code.contains("pub primary_list: Vec<Topic>,"));
    assert!(code.contains("pub backup_list: Vec<Topic>,"));
    assert!(code.contains("pub primary_vector: Vec<Topic>,"));
    assert!(code.contains("pub backup_vector: Vec<Topic>,"));
    assert!(code.contains("pub maybe_primary: Option<Topic>,"));
    assert!(code.contains("pub maybe_backup: Option<Topic>,"));
    assert!(code.contains("pub optional_primary: Option<Topic>,"));
    assert!(code.contains("pub optional_backup: Option<Topic>,"));
    assert!(
        code.contains(
            "pub primary_dictionary: std::collections::BTreeMap<Topic, RecordIdentifier>,"
        )
    );
    assert!(
        code.contains(
            "pub backup_dictionary: std::collections::BTreeMap<Topic, RecordIdentifier>,"
        )
    );
    assert!(code.contains("pub primary_map: std::collections::BTreeMap<Topic, RecordIdentifier>,"));
    assert!(code.contains("pub backup_map: std::collections::BTreeMap<Topic, RecordIdentifier>,"));
    assert!(code.contains("Submit,"));
    assert!(code.contains("Reported,"));
}
